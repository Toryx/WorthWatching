...
export function SidebarDemo({children}:Readonly<{children:React.ReactNode}>) 
{
  ...
  function SidebarInside({children}:Readonly<{children:React.ReactNode}>) {
    ...
    function Example() {
      ...
      return (
          <Dialog open={opendialog} onClose={setOpendialog} className="z-10">
              ...
              <div className="fixed inset-0 z-10 w-screen overflow-y-auto pb-[30vh] sm:pb-0">
    ...
    const links = [
      ...
      {
        label:  (isLoggedIn || session) ? "Logout" : "Login",
        href: (isLoggedIn || session) ? "#" : "login",
        icon: (
    ...
    return (
      <div
        className={cn(
          "rounded-md flex flex-col md:flex-row bg-gray-100 dark:bg-slate-950 w-full flex-1 mx-auto  dark:border-neutral-700 ",
  ...
  return (  <SessionProvider><SidebarInside>{children}</SidebarInside></SessionProvider>)
